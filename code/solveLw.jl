function solveLw(param, a, b, sp, nobs, model)
    xtic = time()
    ...
    # Trade costs
    dd = sp .^ (-theta * phi) # Check this step in the paper (page 158, below Figure 2). # Type: Matrix{Float64}
    ...
    if (wconverge != 1.0) & (Lconverge != 1.0)
        println("Wages and Population failed to converge")
    elseif (wconverge != 1.0)
        println("Wages failed to converge")
    elseif (Lconverge != 1.0)
        println("Population failed to converge")
    end
